...
An explicit dataDir names one document authority, even when the caller's cwd
changes. The default Host adapter maps workspace/global storage as before.
Standalone archival retains content locally; it does not claim cross-Source
LLM distillation. Creating a document through management does not need an Agent.
