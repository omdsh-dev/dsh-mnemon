...
export function inspectMemoryView(generation: MemoryCompositionGeneration, view: ComposableMemoryView, state: MemoryViewInspection['state'], turn?: number, memoryText?: string): MemoryViewInspection {
  return structuredClone({
    ...
    routes: view.routes.map(({ id, sourceInstanceKey, sourceRouteId, description, maxCalls, access }) => ({ id, sourceInstanceKey, operationId: sourceRouteId, description, maxCalls, ...(access === undefined ? {} : { access }) })),
    actions: view.actionOffers.map(({ id, sourceInstanceKey, sourceActionId, description, operation, authority }) => ({ id, sourceInstanceKey, operationId: sourceActionId, description, requiresApproval: authority !== undefined, ...(operation === undefined ? {} : { operation }) })),
    ...(view.decisions === undefined ? {} : { decisions: view.decisions }),
