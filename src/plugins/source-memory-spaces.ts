...
import { BUILTIN_MEMORY_SPACE_PROVIDER_ENTRIES, createMemorySpaceProviderPlugin } from './memory-space-providers.ts'

export const name = 'dsh-mnemon-source-memory-spaces'
export const inject = ['mnemonMemory']

const BUNDLED_PROVIDER_IDS = BUILTIN_MEMORY_SPACE_PROVIDER_ENTRIES.map(entry => entry.instanceId)
...
export const Config = z.object({
  providers: z.array(z.union([z.string(), MemorySpaceProviderDeclarationSchema] as const)).default([...BUNDLED_PROVIDER_IDS]),
...
function bundledProviderEntry(use: string): MemorySpaceProviderEntry | undefined {
  const normalized = use.trim()
  const id = normalized.startsWith('dsh-mnemon-provider-')
    ? normalized.slice('dsh-mnemon-provider-'.length)
    : normalized
  return BUILTIN_MEMORY_SPACE_PROVIDER_ENTRIES.find(candidate => candidate.instanceId === id)
}
...
export async function resolveMemorySpaceProviderEntries(
  ...
  for (const declaration of declarations) {
    ...
    const bundled = bundledProviderEntry(use)
...
export async function installMemorySpaces(
  ctx: Context,
  entries: readonly MemorySpaceProviderEntry[] = BUILTIN_MEMORY_SPACE_PROVIDER_ENTRIES,
  ...
  const children = entries.map(entry => ctx.plugin(createMemorySpaceProviderPlugin(entry, privateHost), entry.config))
  ...
  installMemory(ctx, { sources: [createMemorySpacesSource(snapshot)] }, {
    instanceId: entryId,
    effectiveDigest: `providers:${snapshot.digest}`,
  })
  return snapshot
}

export async function apply(ctx: Context, config: Config = { providers: [...BUNDLED_PROVIDER_IDS] }): Promise<void> {
  await installMemorySpaces(ctx, await resolveMemorySpaceProviderEntries(ctx, config.providers))
}
