...
`tests/bundle-activation.spec.mjs` boots the published DSH Loader with disposable activation counters for all seventeen Starter packages. The pinned host checks the legacy `mnemon` configuration/disable target and independent component choices. Set `MNEMON_BUNDLE_TEST_PROFILE=/absolute/profile` to also use that installation's published plugin manager (verified on `0.1.7-rc.2`): real component and bundle toggles must persist across reloads and restarts, the core gate must stop every active child, and no package may leave duplicate instances. The fixture creates its own temporary profile and does not edit the supplied installation. It tests lifecycle contracts; packed Headless and WebUI checks still own real Mnemon behavior. Run `node --expose-internals tests/fixtures/bundle-activation.mjs /absolute/profile manager --check-declared-rows` to additionally require every advertised bundle component to be manageable. That diagnostic fails on the known `0.1.7-rc.2` native-group listing defect described in [Compatibility](../reference/compatibility.md#dsh-017-bundle-component-list).
