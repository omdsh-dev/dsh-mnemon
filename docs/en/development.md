...
## Coordinated prereleases

`pnpm release:check` is read-only. It checks the Root and all sixteen official plugins, exact internal dependency/peer versions, repository metadata, the release tag and `publishConfig.tag`. This coordinated version policy applies to the official distribution, not third-party repositories. Prerelease dependencies pin the tested version explicitly; `^0.4.0` cannot install a `0.5.0-rc.1` SDK.

The manually dispatched npm workflow accepts a full commit SHA that must already equal `main`. Before requesting any npm credential, it runs `verify`, the independent-plugin suite, and a real packed `v0.4.7` upgrade, then packs all seventeen artifacts once and records their revision, byte size, and SHA-512 integrity. The protected `npm-release` Environment gates Registry writes. After approval, the workflow publishes the sixteen plugins serially, reads them back and installs them through the frozen local Starter, publishes the Starter, verifies a complete Registry install, and runs a real Registry `v0.4.7` upgrade. Only then does it create the GitHub prerelease.
