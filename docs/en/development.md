...
`pnpm release:check` is read-only. It checks the Root and all sixteen official plugins, exact internal dependency/peer versions, repository metadata, the release tag and `publishConfig.tag`. This coordinated version policy applies to the official distribution, not third-party repositories. All official packages pin the tested cohort explicitly; stable releases use `latest`, while prereleases use their named channel.

The manually dispatched npm workflow accepts a full commit SHA that must already equal `main`. Before requesting any npm credential, it runs `verify`, the independent-plugin suite, and a real packed `v0.4.7` upgrade, then packs all seventeen artifacts once and records their revision, byte size, and SHA-512 integrity. The protected `npm-release` Environment gates Registry writes. After approval, the workflow publishes the sixteen plugins serially, reads them back and installs them through the frozen local Starter, publishes the Starter, verifies a complete Registry install, and runs a real Registry `v0.4.7` upgrade. Only then does it create the matching GitHub prerelease or stable Release.

Interrupted runs are resumable: an existing version is skipped only when its Registry integrity exactly matches the frozen tarball. A different artifact at the same version stops the release. Prereleases require an explicit `alpha`, `beta`, or `rc` npm tag; stable releases require `latest`. Do not publish only the Root.
