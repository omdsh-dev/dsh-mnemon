...
This checkout targets the composable v0.4.0 distribution; its Root and thirteen plugin artifacts have not been published as part of this work. The registry examples below describe the previously published v0.3.5 installation experience. To test this checkout, use the [development verification fixtures](./development.md). The pinned local baseline is DSH 0.1.1-rc.2 and Mnemon 0.2.3; some retained screenshots come from dsh-mnemon v0.2.0. DSH rc.2 uses `Promise.withResolvers` and the Node Zstd API, so Node 20 cannot boot its complete profile. CI separately covers the source-only DSH 0.1.2-alpha.1 profile. Back up and repeat verification against an isolated root before upgrading.
