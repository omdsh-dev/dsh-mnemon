...
- **v0.4：Sidebar 唯一入口。** 移除 builtin 展示模式和展示形态配置，保留 v0.3 架构与记忆数据格式。参见 [v0.4.0 发布说明](./releases/v0.4.0.md)。
- **v0.5：计划中的完整 view-based 升级。** 在 Sidebar 唯一入口基础上独立开发和验证架构升级。这项工作不包含在 v0.4.0 中，发布 v0.4 不代表新架构已就绪。
