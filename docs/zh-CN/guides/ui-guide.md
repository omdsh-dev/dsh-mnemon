...
设置沿用 DSH 的紧凑行布局、分段选项、开关、分隔线和主题颜色。展开“组合策略配置”可选择主策略，并逐项展开增强功能。此编辑器中的开关先修改草稿；点击“预览组合”，再“保存这份组合”后应用于新轮次。继续编辑会使旧预览失效，来源操作和插件标识按需展开。

当宿主导航留给内容的宽度小于 260 像素时，“打开记忆设置”通过 DSH 的公开对话框组件打开完整表单。滚动时标题与关闭按钮保持可见；Escape 返回原设置页并恢复焦点，基础设置草稿保留。

设置页集中管理稳定的用户选项与可复用的**服务配置**：

- 记忆层设置行来自运行中的 Catalog；Runtime、Documents、Memory Spaces 各有一个总开关，没有额外的参与模式选项；
