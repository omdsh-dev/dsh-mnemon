...
云端回滚到 rc.2 时所需的 `remoteAccess` 覆盖就属于这种整行自定义；稳定版 DSH 0.1.2-rc.1 不需要该覆盖。确需 rc.2 时，请使用[云端 WebUI 操作步骤](../guides/operations.md#cloud-hosted-webui)中的完整、可随升级核对的示例，不要只写 `config: { remoteAccess: trusted-host }` 片段。
