...
系统不再提供独立的“视图”页面。从状态页打开**记忆插件**；最终 View 仍是为某个 LLM 回合编译并固定的运行时产物，不成为另一份需要用户维护的文档。

- **Source**：列出每个已注册 Source Entry 及 Core 实际观察到的运行状态。开关只改变该 Source，核对其真实贡献后保存当前 Profile 的明确选择；关闭不会删除 Source 数据。
- **基础与叠加 Strategy**：选择一个完整 Strategy，并独立开启兼容的扩展。编辑字段由插件自己声明，例如 Source 顺序、可写子集、常驻内容上限或记录指引。“使用插件默认值”会删除覆盖值。
- **保存组合**：先校验完整候选，再修改 Entry。冲突、过期修订、激活失败或持久化失败都会反馈并回滚；无关插件保持启用。保存结果只影响后续回合。
- **发现**：只接受准确的 `dsh-mnemon-source-*` 或 `dsh-mnemon-strategy-*` npm 包名。页面先检查身份与兼容性，再通过第二次明确确认让 DSH 把准确版本安装进当前 Profile。重启后 bundle 以停用状态注册，激活仍是独立选择。

这里是有边界的安装入口，不是经过策展的插件市场。未提供可选 Strategy 编辑描述的包仍能通过普通 DSH 配置使用，并在弹窗中只读呈现。默认 Starter 不安装三个示例叠加插件。作者接入方式见[插件自有配置元数据](./extensions.md)。
