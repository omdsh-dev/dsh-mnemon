...
本指南按 Sidebar 优先体验和真实用户路径编排。Composable Source 架构保持既有交互流程；可选 Builtin 入口将同一组页面放入会话，不另建布局。v0.5 相对 v0.4 的可见变化仅是“设置 → 记忆系统”新增三个记忆增强开关，不增加 View 页或通用插件管理入口。历史截图和录制来自 v0.2.0 的 1600×900 实机页面；[入口位置设置截图](../assets/screenshots/settings-entry-placement.png)记录 v0.4.2 恢复的选项。名称、数量和内容会随本地数据变化。
...
一级页仍按**状态、运行时、档案、记忆体**排列。记忆体内部再分为**概览、检索、内容、实体**，右上角保留“沉淀记忆”和“沉淀策略”。生成的 View 是每轮内部运行时产物，不单独占用导航页面，状态页也不承担插件发现或安装。
...
开关立即应用于后续回合；当前已经固定的回合不被改写。三个增强可以同时开启，关闭不会删除 Source 数据。界面只表达用户可感知的行为，不显示包名、Entry、依赖或互斥关系。第三方 Source/Strategy 继续通过 DSH Profile/Loader 安装和装配；作者规范与贡献入口见[记忆插件开发](./extensions.md)。
...
- 三个记忆增强开关即时应用；其余改动由底部保存动作统一写入，不在设置页等待完整发现或检索；健康监控放在状态页，记忆体发现放在概览；
