...
本指南按 Sidebar 优先体验和真实用户路径编排。Composable Source 架构保持既有交互流程；可选 Builtin 入口将同一组页面放入会话，不另建布局。历史截图和录制来自 v0.2.0 的 1600×900 实机页面；[入口位置设置截图](../assets/screenshots/settings-entry-placement.png)记录 v0.4.2 恢复的选项，当前组合控制在“设置”部分说明。名称、数量和内容会随本地数据变化。
...
使用 `displayMode: builtin` 时，从会话 Tab 打开“记忆系统”，Sidebar 不再显示入口。Host 使用所属会话的全局、工作区或自定义范围，因此顶部不显示存储模式和工作区选择器。以下 Source 页面与弹窗全部共用，对话快捷入口会打开匹配的 Tab。详见[范围映射](./configuration.md#入口位置displaymode-与-tabenabled)。

一级页按**状态、运行时、档案、记忆体**排列。记忆体内部再分为**概览、检索、内容、实体**，右上角保留“沉淀记忆”和“沉淀策略”。状态页通过**记忆插件**弹窗完成 Source 的发现/激活与 Strategy 组合；生成的 View 保留为运行时概念，不再单独占用导航页面。

| 看到的动作 | 点击后发生什么 | 是否启动独立任务 Agent |
|---|---|---|
| 刷新状态、立即同步、点击记忆体卡片重连 | Host 异步读取；当前区域右上角或卡片状态灯显示一个转圈 | 否 |
| 打开记忆插件 | 查看已注册 Source 与 Strategy、发现准确包名，或组合后续轮次 | 不调用模型、不整理或写入记忆 |
| 保存组合 | 校验完整组合，更新已有 DSH 插件 Entry，并保存当前 Profile 的配置 | 否；本轮仍使用原快照 |
...
中文 Tab 为 **视图**，英文为 **View**。沿用其他页面的底色、字体和控件；宽屏中查看区和配置区并排，窄屏自动上下排列。

- **实际 View**：展示本轮固定快照，或当前运行的 Host 保留的最近一轮快照。可以查看 Source 内容片段、常驻字符数、读取入口、写入动作、策略指引与快照标识。重新打开侧栏会刷新快照而不丢弃草稿，也可以点击“刷新视图状态”。尚未生成、工作区不一致、Host 重启后没有快照时显示原因，不虚构历史上下文。
- **配置预览**：用当前 Source 状态编译草稿，不激活插件、不执行 Action、不调用模型或改写记忆。预览不是已经注入的 View；实际执行前 Source 发生变化，结果也可能不同。
- **策略组合**：选择一个基础 Strategy，独立开启需要的叠加插件。插件声明各自的字段，例如 Source 顺序与可写范围、常驻内容预算、记录指引。“使用插件默认值”会删除覆盖值；显式选择零个可写 Source 则表示禁止写入。
- **应用到下一轮**：先校验完整组合，再修改 Entry。冲突、过期配置、插件激活或持久化失败都会反馈；不会因为启停一项而关闭无关插件。配置保存到当前 DSH Profile，供该 Profile 各工作区的后续轮次使用，重启后恢复；不修改正在执行的本轮及其既有权限。

这里不是插件市场。可选的 `dsh-mnemon-strategy-*` 插件先通过 DSH 安装并登记为 Entry；已登记但禁用的 Entry 可以在本页开启。没有提供可选配置描述的插件仍可使用正常的 DSH 配置，本页只读展示。默认 Starter 不自动安装三个可选叠加插件。作者接入方式见[插件自有配置元数据](./extensions.md)。

字符数只统计内容投影，**不是 token 数或完整模型请求长度**。原始记忆快照不包含完整对话、工具目录或 system prompt；展示的策略指引仍受 Host 注入设置约束。主动记录只指导当前 LLM，不启动自主后台 Agent。
