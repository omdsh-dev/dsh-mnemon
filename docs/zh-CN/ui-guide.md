...
一级页按**状态、视图、运行时、档案、记忆体**排列。记忆体内部再分为**概览、检索、内容、实体**，右上角保留“沉淀记忆”和“沉淀策略”。上面的旧版录屏尚未包含“视图”Tab。

| 看到的动作 | 点击后发生什么 | 是否启动独立任务 Agent |
|---|---|---|
| 刷新状态、立即同步、点击记忆体卡片重连 | Host 异步读取；当前区域右上角或卡片状态灯显示一个转圈 | 否 |
| 查看视图 / 生成预览 | 读取真实固定快照，或根据当前 Source 编译带有明确标识的只读候选 | 不调用模型、不整理或写入记忆 |
| 应用到下一轮 | 校验完整组合，更新已有 DSH 插件 Entry，并保存当前 Profile 的配置 | 否；本轮仍使用原快照 |
