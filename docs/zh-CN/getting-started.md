...
当前预发布版是 composable v0.5.0-rc.1：一个 Starter 与十六个锁定同版本的官方插件。请先阅读 [RC 范围与升级说明](./releases/v0.5.0-rc.1.md)。DSH 锁定基线是稳定版 0.1.2-rc.1，Native 集成另经 Mnemon 0.2.5 验证。部分保留截图来自 dsh-mnemon v0.2.0。完整 DSH rc.1 profile 需要 Node `^22.19.0 || >=24.0.0`，Mnemon 包仍为较旧且兼容的 Host 保留 Node 20 engine 下限。CI 另从源码覆盖 DSH 0.1.2-alpha.5 前序版本，并保留对 0.1.1-rc.2 的回滚回归。升级前先备份，并在隔离目录重复验证。
