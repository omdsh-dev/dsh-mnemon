...
当前稳定版是 composable v0.5.2：一个 Starter 精确固定经过验证的十六个官方插件组合。请先阅读 [v0.5.2 补丁与升级说明](./releases/v0.5.2.md)。DSH 锁定基线是 0.1.2-rc.1，Native 集成另经 Mnemon 0.2.5 验证。部分保留截图来自 dsh-mnemon v0.2.0。完整 DSH rc.1 profile 需要 Node `^22.19.0 || >=24.0.0`，Mnemon 包仍为较旧且兼容的 Host 保留 Node 20 公开入口兼容。升级前先备份，并在隔离目录重复验证。
