import { Config, InteractionConfig, resolveConfig, resolveInteractionConfig, type Config as MnemonConfig } from '../../src/config.ts'
import { registerCommands } from '../../src/commands.ts'
import type { HostContextShape } from '../../src/contracts.ts'
import { DocumentManager } from '../../src/documents.ts'
import { registerGuidance, registerMemoryPromptInterpolation } from '../../src/guidance.ts'
...
import { MnemonPackManager } from '../../src/pack.ts'
import { VersionUpdateManager } from '../../src/version-updates.ts'
import { registerMnemonSubagentTokenUsageProjection } from '../../src/subagent-token-usage.ts'
import type { HostWorkspaceRegistry } from '../../src/contracts.ts'
import { MemoryBoot, MemoryExtensionHost, MemoryRuntime, memoryBoot } from '../../packages/extension-sdk/src/index.ts'
import { installBundledComposableMemory } from './contributions.ts'
import { provideMemoryRuntime } from '../../src/core.ts'
...
/** Default DSH host adapter and legacy three-tier compatibility assembly. */
export interface ApplyDefaultBundleOptions {
  /** Include the one-Entry shortcut; the explicit five-Entry starter sets false. */
  compatibilityBundle: boolean
}

export function applyDefaultMemoryBundle(rawContext: unknown, config: MnemonConfig = {}, options: ApplyDefaultBundleOptions = { compatibilityBundle: true }): void {
  const ctx = rawContext as unknown as HostContextShape
  registerMnemonSubagentTokenUsageProjection(ctx)
  const extensions = provideMemoryRuntime(ctx, memoryBoot)
...
export function apply(rawContext: unknown, config: MnemonConfig = {}): void {
  applyDefaultMemoryBundle(rawContext, config, { compatibilityBundle: config.bundledContributions !== false })
}
