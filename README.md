...
This branch prepares **v0.5.0-beta.1: View-based Memory and Source/Strategy plugins**. The SDK is beta, not a frozen extension ABI; the default user configuration and workflows remain compatible. Compared with v0.4, the ordinary UI adds only three disabled-by-default enhancement switches under **Settings → Memory System**—no View page or plugin-management entry. Development verification does not publish these artifacts. See [beta scope and upgrade notes](./docs/en/releases/v0.5.0-beta.1.md); v0.4 remains the separate Sidebar-convergence release line.
